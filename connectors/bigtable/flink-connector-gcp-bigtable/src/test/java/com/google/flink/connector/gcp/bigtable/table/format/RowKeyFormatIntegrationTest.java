...
public class RowKeyFormatIntegrationTest {
    ...
    @Test
    public void testZeroPaddedWithExplicitField() {
        Map<String, String> options = getRequiredOptions();
        options.put(BigtableConnectorOptions.COLUMN_FAMILY.key(), TestingUtils.COLUMN_FAMILY);
        options.put("key.format", "zero-padded");
        options.put("key.zero-padded.field", "shop_id");
        options.put("key.zero-padded.pad-length", "20");

        ResolvedSchema schema = getExplicitFieldSchema();
    ...
    @Test
    public void testToStringWithExplicitField() {
        Map<String, String> options = getRequiredOptions();
        options.put(BigtableConnectorOptions.COLUMN_FAMILY.key(), TestingUtils.COLUMN_FAMILY);
        options.put("key.format", "to-string");
        options.put("key.to-string.field", "shop_id");
    ...
    @Test
    public void testCompositeZeroPaddedWithExplicitFields() {
        Map<String, String> options = getRequiredOptions();
        options.put(BigtableConnectorOptions.COLUMN_FAMILY.key(), TestingUtils.COLUMN_FAMILY);
        options.put("key.format", "composite-zero-padded");
        options.put("key.composite-zero-padded.fields", "shop_id,product_id");
        options.put("key.composite-zero-padded.pad-length.shop_id", "10");
        options.put("key.composite-zero-padded.pad-length.product_id", "10");

        ResolvedSchema schema = getExplicitCompositeFieldSchema();
    ...
    @Test
    public void testExplicitFieldProducesCorrectRowKey() {
        // Schema: location_id (index 0, PK), shop_id (index 1), name (index 2)
        // key.zero-padded.field = shop_id -> should use index 1
        ZeroPaddedRowKeyFormat format = new ZeroPaddedRowKeyFormat(1, new BigIntType(), 20);

        GenericRowData row = new GenericRowData(3);
        row.setField(0, 100L); // location_id (PK, should be ignored)
        row.setField(1, 42L); // shop_id (explicit row key field)
        row.setField(2, StringData.fromString("test"));

        Assertions.assertThat(format.format(row)).isEqualTo("00000000000000000042");
    }

    @Test
    public void testExplicitFieldNotFoundThrows() {
        Map<String, String> options = getRequiredOptions();
        options.put(BigtableConnectorOptions.COLUMN_FAMILY.key(), TestingUtils.COLUMN_FAMILY);
        options.put("key.format", "zero-padded");
        options.put("key.zero-padded.field", "nonexistent");
        options.put("key.zero-padded.pad-length", "20");
    ...
    private static ResolvedSchema getExplicitFieldSchema() {
        List<Column> columns =
                Arrays.asList(
                        Column.physical("location_id", DataTypes.BIGINT().notNull()),
                        Column.physical("shop_id", DataTypes.BIGINT()),
                        Column.physical("name", DataTypes.STRING()));
        return new ResolvedSchema(
                columns,
                Collections.emptyList(),
                UniqueConstraint.primaryKey("pk", Arrays.asList("location_id")));
    }

    private static ResolvedSchema getExplicitCompositeFieldSchema() {
        List<Column> columns =
                Arrays.asList(
                        Column.physical("location_id", DataTypes.BIGINT().notNull()),
                        Column.physical("shop_id", DataTypes.BIGINT()),
                        Column.physical("product_id", DataTypes.BIGINT()),
                        Column.physical("name", DataTypes.STRING()));
        return new ResolvedSchema(
                columns,
                Collections.emptyList(),
