...
public class BigtableDynamicTableSink implements DynamicTableSink {
    ...
    public SinkRuntimeProvider getSinkRuntimeProvider(Context context) {
        ...
        if (valueEncodingFormat != null) {
            ...
            String rowKeyField = pkIndices.length > 0
                    ? resolvedSchema.getColumn(pkIndices[0]).get().getName()
                    : null;
        ...
        long flushMaxBytes = connectorOptions.get(BigtableConnectorOptions.FLUSH_MAX_BYTES);
        if (flushMaxBytes > 0) {
            sinkBuilder.setFlushMaxBytes(flushMaxBytes);
        }

        long flushInterval = connectorOptions.get(BigtableConnectorOptions.FLUSH_INTERVAL);
        if (flushInterval > 0) {
            sinkBuilder.setFlushInterval(flushInterval);
        }
    ...
    public String asSummaryString() {
        return String.format(
                "BigtableSink("
                        ...
                        + "connectorOptions=%s, "
                        + "resolvedSchema=%s)",
                parallelism, connectorOptions, resolvedSchema);
    }
    ...
    private Map<String, SerializationSchema<RowData>> buildFamilySerializers(
        ...
        for (Field field : DataType.getFields(physicalSchema)) {
            if (!field.getDataType().getLogicalType().is(LogicalTypeRoot.ROW)) {
                continue; // Skip scalar fields (including row key) — only ROW fields are column
                // families
            }
