...
public class ZeroPaddedRowKeyFormatFactory implements BigtableRowKeyFormatFactory {
    ...
    public static final ConfigOption<String> FIELD =
            ConfigOptions.key("key.zero-padded.field")
                    .stringType()
                    .noDefaultValue()
                    .withDescription(
    ...
    public BigtableRowKeyFormat createRowKeyFormat(
        ...
        if (fieldName != null) {
            ...
            fieldIndex = idx;
            type = schema.getColumn(idx).get().getDataType().getLogicalType();
        } else {
            int[] pkIndices = schema.getPrimaryKeyIndexes();
            if (pkIndices.length != 1) {
                throw new ValidationException(
                        String.format(
                                "'zero-padded' key format requires exactly one primary key column (or set '%s'), but found %d.",
                                FIELD.key(),
                                pkIndices.length));
            }
            fieldIndex = pkIndices[0];
            type = schema.getColumn(fieldIndex).get().getDataType().getLogicalType();
        }
        ...
        return new ZeroPaddedRowKeyFormat(fieldIndex, type, padLength);
    }
}
